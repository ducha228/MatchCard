...
-(void) lose
{
    if(_timer)
    {
        ...
        [loseAlert show];
        _rePlay = [UIButton buttonWithType:UIButtonTypeRoundedRect];
        _rePlay.frame = CGRectMake(90, 400, 140, 40);
...
-(void) win
{
    if(_timer)
    {
        ...
        _nextLevel = [UIButton buttonWithType:UIButtonTypeInfoLight];
        _nextLevel.frame = CGRectMake(90, 400, 140, 40);
...
-(void) nextLevel
{
    Level2 *level2 = [[Level2 alloc] initWithNibName:@"Level2" bundle:nil];
    [self presentViewController:level2 animated:YES completion:^{
